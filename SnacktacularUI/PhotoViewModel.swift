...
class PhotoViewModel{
    ...
    static func saveImage(spot: Spot, photo: Photo, data: Data) async{
        ...
        metadata.contentType = "image/jpg"  //  will allow imaage to be viewed in the browser from Firestore console
        ...
        do{
            ...
            do{
                try
                db.collection("spots").document(id).collection("photos").document(photo.id ?? "n/a").setData(from: photo)
            } catch{
