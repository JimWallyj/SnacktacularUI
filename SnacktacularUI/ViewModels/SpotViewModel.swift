...
class SpotViewModel: ObservableObject{
    ...
    if photo.id != nil{
        photoName = photo.id!  //  I have a photo.id, so use this as the photoName.  This happens if we're updating an existing Photo's descriptio info. It'll resave the photo, but that's OK. It'll just overwrite the existing one.
    }
